...
class PredictionModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels=1024, aspect_ratios=[[1]], scales=[1], parent=None, index=0):
        ...
        self.num_priors  = sum(len(x)*len(scales) for x in aspect_ratios) # =9
        ...
        if parent is None:
            if cfg.extra_head_net is None:
                ...
                self.upfeature, out_channels = make_net(in_channels, cfg.extra_head_net) # 차원 수 변화없음. conv2d(f=3, p=1)
                # out_channels = 256.
            ...
            self.bbox_layer = nn.Conv2d(out_channels, self.num_priors * 4,                **cfg.head_layer_params) #out channel:(9*4)
            self.conf_layer = nn.Conv2d(out_channels, self.num_priors * self.num_classes, **cfg.head_layer_params) #out channel:(9*c)
            self.mask_layer = nn.Conv2d(out_channels, self.num_priors * self.mask_dim,    **cfg.head_layer_params) #out channel:(9*32) - coefficient
    ...
    def make_priors(self, conv_h, conv_w, device):
        ...
        with timer.env('makepriors'):
            if self.last_img_size != (cfg._tmp_img_w, cfg._tmp_img_h):
                ...
                for j, i in product(range(conv_h), range(conv_w)):
                    ...
                    for ars in self.aspect_ratios:
                        for scale in self.scales:
                            for ar in ars:
                                if not cfg.backbone.preapply_sqrt: # not False
                                    ar = sqrt(ar)

                                # True
                                if cfg.backbone.use_pixel_scales: #original image에 맞추어 box만들기.
                ...
                self.priors.requires_grad = False #  torch.Tensor를 생성하면 주의.
                self.last_img_size = (cfg._tmp_img_w, cfg._tmp_img_h) # (550, 550)
                self.last_conv_size = (conv_w, conv_h)  # (69, 69) -> -> (4, 4)
...
class FPN(ScriptModuleWrapper):
    ...
    #   yolact_plus에서 [512, 1024, 2048] 넘겨줌.
    def __init__(self, in_channels):
        super().__init__()

        self.lat_layers  = nn.ModuleList([
            nn.Conv2d(x, cfg.fpn.num_features, kernel_size=1)
            for x in reversed(in_channels) #[2048, 102, 512]
        ])
        #   self.lat_layers = [
        #                   Conv2d(2048, 256, k=1),
        #                   Conv2d(1024, 256, k=1),
        ...
        # 역전파시 호환되도록 여기에 추가됨.
        # True
        padding = 1 if cfg.fpn.pad else 0 # padding = 1.
        ...
        if cfg.fpn.use_conv_downsample:#True
            self.downsample_layers = nn.ModuleList([
                ...
                for _ in range(cfg.fpn.num_downsample)  # num_downsample = 2
            ])

        # self.downsample_layers = [
        #                   Conv2d(256, 256, k=3, p=1, s=2),
        #                   Conv2d(256, 256, k=3, p=1, s=2),
        #               ]
    ...
    def forward(self, convouts:List[torch.Tensor]):
        ...
        for _ in range(len(convouts)):
            out.append(x)
        # convouts[0].size() = (n, 512, 69, 69)
        # convouts[1].size() = (n, 1024, 34, 34)
        # convouts[2].size() = (n, 2048, 17, 17) 
        ...
        for lat_layer in self.lat_layers:
            ...
            if j < len(convouts) - 1: # j = 2일 경우 skip.
                _, _, h, w = convouts[j].size()
                x = F.interpolate(x, size=(h, w), mode=self.interpolation_mode, align_corners=False)
            
            # lay_layer(convouts[2]) = (n, 256, 69, 69)
            x = x + lat_layer(convouts[j])
            out[j] = x
        #   out[2] = P5 = lat_layers[0](convouts[2])                    #(n, 256, 17, 17)
        #   out[1] = P4 = interpolate(P5) + lat_layer[1](convouts[1])   #(n, 256, 34, 34)
        #   out[0] = P3 = interpolate(P4) + lat_layer[2](convouts[0])   #(n, 256, 69, 69)
        ...
        for pred_layer in self.pred_layers:
            ...
            if self.relu_pred_layers:#True
                F.relu(out[j], inplace=True)
        #   out[2] = pred_layers[0](P5) 
        #   out[1] = pred_layers[1](P4) 
        #   out[0] = pred_layers[2](P3)

        cur_idx = len(out) #3

        # 논문에서의 P6, P7층을 생성하는 과정.
        # In the original paper, this takes care of P6
        if self.use_conv_downsample: #True
            for downsample_layer in self.downsample_layers:
                out.append(downsample_layer(out[-1]))
            #   out[3] = downsample_layer[0](out[2]) #(n, 256, 8, 8)
            #   out[4] = downsample_layer[1](out[3]) #(n, 256, 4, 4) 
        else:
            for idx in range(self.num_downsample):
                # Note: this is an untested alternative to out.append(out[-1][:, :, ::2, ::2]). Thanks TorchScript.
                out.append(nn.functional.max_pool2d(out[-1], 1, stride=2))

        if self.relu_downsample_layers: #False
...
#ScriptModuleWrapper = torch.jit.ScriptModule : pytorch 코드를 C++로 컴파일하기 위함.
class FastMaskIoUNet(ScriptModuleWrapper):
#'maskiou_net': [
#       (8, 3, {'stride': 2}), (16, 3, {'stride': 2}), 
#       (32, 3, {'stride': 2}), (64, 3, {'stride': 2}), 
#       (128, 3, {'stride': 2})                         ],
    def __init__(self):
        super().__init__()
        input_channels = 1
...
class Yolact(nn.Module):
    ...
    def __init__(self):
        ...
        if cfg.fpn is not None:
            ...
            self.fpn = FPN([src_channels[i] for i in self.selected_layers]) #[512, 1024, 2048] 넘김.
            self.selected_layers = list(range(len(self.selected_layers) + cfg.fpn.num_downsample)) #cw range(3 + 2)
    ...
    def forward(self, x):
        ...
        if cfg.mask_type == mask_type.lincomb and cfg.eval_mask_branch: #True
            with timer.env('proto'):
                proto_x = x if self.proto_src is None else outs[self.proto_src] # proto_x = outs[0] = P3
                ...
                #   proto_x = (n, 256, 69, 69)
                proto_out = self.proto_net(proto_x) # P3를 proto_net에 통과. -> (n, 32, 138, 138)
                ...
                proto_out = proto_out.permute(0, 2, 3, 1).contiguous() # (n, h, w, channel)로 변환. ->(n, 138, 138, 32)
        ...
        with timer.env('pred_heads'):
            ...
            for idx, pred_layer in zip(self.selected_layers, self.prediction_layers): # 5개 나오는 prediction layers
                pred_x = outs[idx]
                ...
                if cfg.share_prediction_module and pred_layer is not self.prediction_layers[0]: #1번째 꺼 제외 True.

...
class MultiBoxLoss(nn.Module):
    ...
    Compute Targets: [cw : 연산하고자 하는 target]
        1) Produce Confidence Target Indices by matching  ground truth boxes
           ...
           : 생성된 anchor boxes는 Target indices라고도 할 수 있으며, 각 Target index는 ground truth와 비교하여
             연산된 jaccard index값을 가지고, 이 jaccard index가 설정한 threshold보다 높은 경우인 Confidence한 target을 가려내는 것이

        2) Produce localization target by 'encoding' variance into offsets of ground
           truth boxes and their matched  'priorboxes'.
           : Ground truth와 priorboxes의 offsets로 분산을 'encoding'함으로써 타겟의 로컬라이제이션 수행.(정확히는 이해안감)
    ...
    def __init__(self, num_classes, pos_threshold, neg_threshold, negpos_ratio):
        ...
        #cw : 만약 class가 balance하게 나오도록 추가 설정을 했다면, instance의 숫자를 Counting하기 위한 변수 생성.
    ...
    def forward(self, net, predictions, targets, masks, num_crowds):
        """Multibox Loss
        Args:
            [cw : loc(location), conf(confidence), pred(prediction)
                priorboxes(SSD의 default boxes, R-CNN의 Anchor boxes.)]
